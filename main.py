...
async def info(ctx, arg, *options):
    ...
    embed = discord.Embed(title=pokemon+" :japanese_goblin:", color=discord.Color.from_rgb(204, 0, 0)) # add title and color
    embed.add_field(name='what the thing do?', value=find_first_en()['flavor_text'])
    embed.add_field(name='Stats', value=pkmn_stats, inline=False)

    options=["abilities", "types"]
    ...
    embed.set_footer(text=f'{pokemon}', icon_url=pokeball_url)# create a footer with pokeball and name
    embed.set_thumbnail(url=sprite) # set a thumbnail for our embed

    # step 4: send embed to server
    await ctx.send(embed=embed) 

bot.run("MTAzMjc5NDg1NjAwODk3NDM5Ng.GeGkOr.RxyQxm5jqvwotyNh6OHLU6gJaAtzNTfG7PvwQw")